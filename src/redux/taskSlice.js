...
export const addTask = (task, id) => async (dispatch) => {
  ...
  const response = await axios.post(`${API_URL}/task/add`, taskData);
  if (response) {
...
export const getAllTasks = (token, id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(`${API_URL}/task/tasks`, config);
    if (response) {
...
export const arrowClick = (item, string) => async () => {
  ...
  try {
    let response = await axios.put(`${API_URL}/task/${taskData.id}`, taskData);
...
export const deleteItem = (id) => async (dispatch) => {
  let res = await axios.delete(`${API_URL}/task/${id}`);
